...
"""
    We run a simulation study to test how the confidence intervals proposed in the thesis perform, in particular, we want to investigate their coverage.
    There are 4 methods tested: The two analytical versions using the all observations or only the estimated inliers of the transformed sample, bootstrapping and double bootstrapping.
    The experiment can take several hours to run, therefore the values are already saved. To rerun the experiment, set the "run_exp" variable to True.
...
if run_exp:
    ...
    with open(path_results+"confidence_interval_t" + str(data_args["beta"])+ '.pkl', 'wb') as fp:
